...
namespace PS3TrophiesIsPerfect.Models
{
    ...
    public sealed class TrophyDetail : INotifyPropertyChanged
    {
        ...
        public string EarnedAgo => Earned && EarnedUtc.HasValue ? Ago(EarnedUtc.Value.ToLocalTime()) : "";

        public double EarnedOpacity => Earned ? 1.0 : 0.45;

        // ---- Rarity ----
        public string RarityText => EarnedRate <= 0 ? "" : EarnedRate.ToString("0.#") + "%";
        public string RarityLabel => EarnedRate <= 0 ? ""
            : EarnedRate < 5 ? "Ultra Rare" : EarnedRate < 20 ? "Rare" : EarnedRate < 50 ? "Uncommon" : "Common";
        public Brush RarityBrush =>
            EarnedRate <= 0 ? Muted : EarnedRate < 5 ? Gold : EarnedRate < 20 ? Blue : EarnedRate < 50 ? Green : Muted;
        ...
        private static string Ago(DateTime when)
        {
            ...
            if (days < 30) return (days / 7) + (days < 14 ? " week ago" : " weeks ago");
            if (days < 365) return (days / 30) + (days < 60 ? " month ago" : " months ago");
