...
namespace PS3TrophiesIsPerfect.ViewModels
{
    ...
    public sealed partial class MainViewModel
    {
        public ObservableCollection<ComparisonRow> Comparison { get; } =
            new ObservableCollection<ComparisonRow>();
        ...
        {
            get => _donorTitle;
            set => Set(ref _donorTitle, value);
        }

        private string _donorUser = "";
        public string DonorUser
        {
            get => _donorUser;
            set => Set(ref _donorUser, value);
        }

        private string _donorAvatarUrl = "";
        public string DonorAvatarUrl
        {
            get => _donorAvatarUrl;
            set => Set(ref _donorAvatarUrl, value);
        }
        ...
        {
            get => _verdictText;
            set => Set(ref _verdictText, value);
        }

        private System.Windows.Media.Brush _verdictBrush = GreenBrush;
        public System.Windows.Media.Brush VerdictBrush
        {
            get => _verdictBrush;
            set => Set(ref _verdictBrush, value);
        }

        private static readonly System.Windows.Media.Brush GreenBrush = FrozenBrush(
            0x3F,
            0xB9,
            0x50
        );
        ...
        private void RebuildComparison()
        {
            ...
            if (HasDonor)
                foreach (var r in _doc.BuildComparison(_donor))
                {
                    ...
                    if (r.Match == "exact")
                        exact++;
                    else if (r.Match == "slower")
                        slower++;
                    else if (r.Match == "faster")
                        faster++;
                }
